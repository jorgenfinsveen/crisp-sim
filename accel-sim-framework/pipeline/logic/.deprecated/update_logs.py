...
def collect_instance_stats(root, param_fields, result_fields, allowed_names: list[str], benchmarks: list[str]) -> tuple[list, list, dict]:
    ...
    for d in iter_target_dirs(root, allowed_names, benchmarks):
        out = get_outfile(os.path.join(d, f"{args.target}.o"))
        cnf = get_config(os.path.join(d, 'gpgpusim.config'))
...
def main():
    ...
    global logs, target
    if logs is None:
        logs = new_sim_log(os.path.expandvars(args.logs))
    ...
    _, benchmarks, results = collect_instance_stats(root, param_fields, result_fields, instances, benchmarks)
    ...
    target.date            = convert_date(args.target, "underscore", "default")
    target.benchmarks      = benchmarks
    target.results         = results

    new_logs = {log_name: sim_logs_to_dict(target)}
